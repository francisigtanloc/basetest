...
COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/basetest/backend/requirements/dev.txt /tmp/plugin-dev-requirements.txt
RUN . /baserow/venv/bin/activate && pip3 install -r /tmp/plugin-dev-requirements.txt

COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/basetest/ $BASEROW_PLUGIN_DIR/basetest/
RUN /baserow/plugins/install_plugin.sh --folder $BASEROW_PLUGIN_DIR/basetest --dev
